...
export class DildoGeneration {
    constructor(canvasId, options) {
        ...
        const lightDistanceFactor = 10.0;
     ...
     */
    __performPlaneSlice(latheMesh, latheUnbufferedGeometry, wireframe, useTextureImage, textureImagePath, options) {
        // var epsilon = 0.000001;
        const leftPlane = new THREE.Plane(new THREE.Vector3(0, 0, 1), 0);
        const leftSliceGeometry = GeometryGenerationHelpers.makeSlice(latheUnbufferedGeometry, leftPlane);
        const rightPlane = new THREE.Plane(new THREE.Vector3(0, 0, -1), 0);
        const rightSliceGeometry = GeometryGenerationHelpers.makeSlice(latheUnbufferedGeometry, rightPlane);
        const sliceMaterial = DildoMaterials.createSliceMaterial(useTextureImage, wireframe, textureImagePath);
        // Find points on intersection path (this is a single path in this configuration)
        const planeGeom = new THREE.PlaneGeometry(300, 500);
        ...
        const planeIntersectionPoints = GeometryGenerationHelpers.makeAndAddPlaneIntersection(this, latheMesh, latheUnbufferedGeometry, planeMesh, planeGeom, options);
        ...
        // Array<number[]>
        const connectedPaths = new PathFinder().findAllPathsOnMesh(leftSliceGeometry, planeIntersectionPoints);
        ...
        const triangulatedGeometries = [];
        for (var i = 0; i < connectedPaths.length; i++) {
            const triangulationGeometry = GeometryGenerationHelpers.makePlaneTriangulation(this, leftSliceGeometry, connectedPaths[i], options);
    ...
    generateSTL(options, exporter) {
        ...
        for (var i in this.geometries) {
            if (this.geometries[i].userData["isExportable"] === true) {
                const stlData = exporter.parse(this.geometries[i]);
