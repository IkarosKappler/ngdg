...
var getImageFromCanvas = function (canvas, context, bounds) {
    var w = canvas.width;
    var h = canvas.height;
    var safeBounds = new plotboilerplate_1.Bounds(new plotboilerplate_1.Vertex(Math.max(bounds.min.x, 0), Math.min(bounds.min.y, h + 1)), new plotboilerplate_1.Vertex(Math.max(bounds.max.x, 0), Math.min(bounds.max.y, h + 1)));
