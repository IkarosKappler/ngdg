...
export class Rulers {
  static mmPerUnit: number = 0.5;
  static stepSize: number = 20; // pixels
  static fontSize: number = 10;
  ...
  static drawHorizontalRuler(draw, fill, outline, color) {
    ...
    for (var i = 0; i < horizontalStepCount; i++) {
      draw.line(
        { x: bounds.max.x - i * Rulers.stepSize, y: bounds.max.y + 10 - 3 },
        { x: bounds.max.x - i * Rulers.stepSize, y: bounds.max.y + 10 + 3 },
        color,
        0.5
      );
  ...
  static drawVerticalRuler(draw, fill, outline, color) {
    ...
    for (var i = 0; i < verticalStepCount; i++) {
      draw.line(
        { x: bounds.max.x + 10 - 3, y: bounds.max.y - i * Rulers.stepSize },
        { x: bounds.max.x + 10 + 3, y: bounds.max.y - i * Rulers.stepSize },
        color,
        0.5
      );
      ...
      if (i % 2 === 0) {
        fill.text(
          Number(i * Rulers.stepSize * Rulers.mmPerUnit).toFixed(0) + "mm",
          bounds.max.x + 16,
          bounds.max.y - i * Rulers.stepSize + Rulers.fontSize * 0.25,
          { color: color, fontSize: Rulers.fontSize / fill.scale.x }
        );
      }
    }
  }
