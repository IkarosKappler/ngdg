...
export class PathFinder {
   ...
   * @instance
   */
  private visitedVertices: Set<number>;
   ...
   * @instance
   */
  private unvisitedVertIndices: Set<number>;
   ...
   * @instance
   */
  private numVisitedVertices: number;
  ...
  constructor(epsilon?: number) {
    this.visitedVertices = new Set<number>();
    this.unvisitedVertIndices = new Set<number>(); // <number>
  ...
  findAllPathsOnMesh(unbufferedGeometry: Gmetry, pathVertices: Array<THREE.Vector3>): Array<number[]> {
    ...
    //
    const pathVertIndices: number[] = mapVerticesToGeometryIndices(unbufferedGeometry, pathVertices, this.epsilon);
    const n: number = pathVertIndices.length;
    ...
    for (var f = 0; f < faceCount; f++) {
      if (faceHasVertIndex(unbufferedGeometry, f, unvisitedIndex)) {
        ...
        for (var i = 0; i < pathVertIndices.length; i++) {
          const pathVertIndex: number = pathVertIndices[i];
   ...
   * @returns {boolean}
   */
  private isVisited(vertIndex: number): boolean {
...
const mapVerticesToGeometryIndices = (
  ...
  var pathVertIndices: Array<number> = [];
  for (var i = 0; i < pathVertices.length; i++) {
    const pathVert: THREE.Vector3 = pathVertices[i];
    let foundIndex: number = -1;
    let foundDist: number = epsilon;
  ...
  for (var f = 0; f < unbufferedGeometry.faces.length; f++) {
    if (faceHasVertIndex(unbufferedGeometry, f, currentVertIndex)) {
      ...
      for (var p = 0; p < collectedPaths.length; p++) {
        ...
        const nextPath: number[] = collectedPaths[p];
