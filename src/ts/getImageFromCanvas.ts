...
export const getImageFromCanvas = (canvas: HTMLCanvasElement, context: CanvasRenderingContext2D, bounds: Bounds) => {
  const w = canvas.width;
  const h = canvas.height;

  const safeBounds = new Bounds(
    new Vertex(Math.max(bounds.min.x, 0), Math.min(bounds.max.x, w)),
    new Vertex(Math.max(bounds.min.y, 0), Math.min(bounds.max.y, h))
  );
  ...
  const subContext: CanvasRenderingContext2D = subCanvas.getContext("2d");
