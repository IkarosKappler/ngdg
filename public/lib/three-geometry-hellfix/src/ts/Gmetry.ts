...
import { Group, MorphNormal, MorphTarget, ThreeFactory } from "./interfaces";
...
export class Gmetry {
  ...
  skinWeights: Array<Vector4>; // TODO: is this type correct?
  skinIndices: Array<Vector4>; // TODO: is this type correct?
  ...
  // This is our accessor to the THREE library without bundling it
  ...
  fromBufferGeometry(geometry: BufferGeometry) {
    ...
    for (let i = 0; i < position.count; i++) {
      ...
      if (color !== undefined) {
        scope.colors.push(this.factory.newColor().fromBufferAttribute(color as BufferAttribute, i));
      }
    }
    ...
    const addFace = (a: number, b: number, c: number, materialIndex?: number) => {
      ...
      if (uv !== undefined) {
        scope.faceVertexUvs[0].push([
          factory.newVector2().fromBufferAttribute(uv as BufferAttribute, a),
          factory.newVector2().fromBufferAttribute(uv as BufferAttribute, b),
          factory.newVector2().fromBufferAttribute(uv as BufferAttribute, c)
        ]);
      }

      if (uv2 !== undefined) {
        scope.faceVertexUvs[1].push([
          factory.newVector2().fromBufferAttribute(uv2 as BufferAttribute, a),
          factory.newVector2().fromBufferAttribute(uv2 as BufferAttribute, b),
          factory.newVector2().fromBufferAttribute(uv2 as BufferAttribute, c)
        ]);
      }
    };
  ...
  computeFaceNormals() {
    const cb = this.factory.newVector3(),
      ab = this.factory.newVector3();
  ...
  merge(geometry: Gmetry, matrix: Matrix4, materialIndexOffset: number = 0) {
  ...
  mergeVertices(precisionPoints: number = 4) {
  ...
  setFromPoints(points: Array<Vector2 | Vector3>): Gmetry {
    ...
    for (let i = 0, l = points.length; i < l; i++) {
      ...
      this.vertices.push(this.factory.newVector3(point.x, point.y, (point as any).z || 0));
    }
    return this;
  }
  ...
  static createBufferGeometryFromObject(object: any, factory?: ThreeFactory) {
